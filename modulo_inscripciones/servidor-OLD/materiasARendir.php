<?php 
	...
	*/
	...
	if($fecha_actual<$fecha_inicio || $fecha_actual>$fecha_fin){
		...
		echo json_encode(["mensaje"=>"No se encuentra dentro del periodo de inscripción","fechas"=>[$fecha_actual,$fecha_inicio,$fecha_fin]]);
		die;
	}
	...
	if(isset($_GET["legajo"]) && !empty($_GET["legajo"])){
		...
		if(Alumno::alumnoExiste($legajo)){
			//Si existe creo el objeto alumno y llamo al devuelvo lo que retorna el metodo materiasDisponiblesParaRendir
